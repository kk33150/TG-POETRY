...
POEM_POOL = [
    ...
    {"title": "示儿", "author": "陆游", "dynasty": "宋", "sentences": ["死去元知万事空，但悲不见九州同。", "王师北定中原日定位，家祭无忘告乃翁。"]},
...
def get_complete_poem():
    """测试机制：优先使用不在本地词库、且长于4句的接口诗句。否则启动本地兜底。"""
    
    # 提取本地词库所有已有的诗名，用于去重比对
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if json_data.get("status") == "success":
                ...
                # 🔍 核心去重与质检逻辑：
                # 1. 句子数量必须 >= 4 句
                # 2. 诗的标题不能存在于本地的 50 首核心词库中
                if len(sentences) >= 4 and title not in local_titles:
                    print(f"✨ [测试通过] 抓取到外部全新完整诗句：《{title}》")
                    return {
                        "title": title,
                        "author": author,
                        "dynasty": dynasty,
                    ...
                    print(f"🛑 [去重拦截] 接口返回了词库已有的诗《{title}》，放弃使用。")
                else:
                    print(f"⚠️ [长度拦截] 接口返回字数不足（{len(sentences)}句），放弃使用。")
    except Exception as e:
        print(f"❌ 接口请求异常或超时: {e}")

    # 接口不满足条件，则启用本地兜底
    print("🎲 接口未返回合适新诗，正在从本地大词库随机抽签...")
